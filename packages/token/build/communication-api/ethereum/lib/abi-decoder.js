...
function _decodeMethod(data) {
  ...
  if (abiItem) {
    ...
    let decoded = web3.SolidityCoder.decodeParams(params, data.slice(10));
    return {
      name: abiItem.name,
...
function _decodeLogs(logs) {
  return logs.map(function (logItem) {
    ...
    if (method) {
      ...
      const decodedData = web3.SolidityCoder.decodeParams(dataTypes, logData.slice(2)); // Loop topic and data to get the params
